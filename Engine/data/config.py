...
# engine
from Engine.data.constants import ZERO, EMPTY, NO, YES
...
class File:
    ...
    config_ext: str = 'json'
    config_dir_is_local: bool = True
    
    data: dict[str, dict] = {}
    # paths
    __ENGINE_DATA__: str = os.path.dirname(os.path.abspath(__file__)).removesuffix(
        '\\PyInstaller\\loader'
    ).replace('\\_internal', '', 1)
    ...
    data_path: str = lambda: File.APPLICATION_path if File.config_dir_is_local else File.APPDATA_LOCAL_path
    # names
    DATA_dir: str = rf'gamedata'
    SHADER_dir: str = rf'{MainData.ENGINE_name}/data/shaders'
    TEXTURE_dir: str = rf'{MainData.ENGINE_name}/data/textures'
    # ico settings
    APPLICATION_ICO_dir: str = rf'{MainData.ENGINE_name}/data'
    ...
    def set_default_data(
        ...
        _ = File.data['Screen'].setdefault('monitor', str(Screen.monitor))
    ...
    def load_engine_config(name: str = 'settings') -> None:
        from Engine.data.constants import _set_repl
        from fnmatch import fnmatch
        # reading
        with open(rf'{File.APPLICATION_path}/{MainData.ENGINE_name}/data/{name}.engconf', 'r') as config_file:
        ...
        for line_index in range(len(lines_data)):
            ...
            if lines_data[line_index] == _set_repl['<ignore message>']:
                continue
            # if the next line is ignored, mark it as ignored and skip the current line
                ...
                lines_data[line_index] = _set_repl['<ignore message>']
                lines_data[line_index + 1] = _set_repl['<ignore message>']
                continue
            else:
                # in other cases, we first look for a reference in <main>
                for line_reference in _set_repl['<main>']:
                    if fnmatch(lines_data[line_index], line_reference):
                        lines_data[line_index] = _set_repl['<ignore message>']
                        break
                else:
                    if name in _set_repl.keys():
                        # if you haven't found a reference among the main ones and the name is on the list, go through
                        for line_reference in _set_repl[name]:
                            if fnmatch(lines_data[line_index], line_reference):
                                lines_data[line_index] = _set_repl['<ignore message>']
                                break
